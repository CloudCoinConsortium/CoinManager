...
func NewConvert(progressChannel chan interface{}) *Convert {
	return &Convert{
		*NewServant(progressChannel),
	}
}

func (v *Convert) Convert(ctx context.Context, coins []cloudcoin.CloudCoin) (*ConvertOutput, error) {
	logger.L(ctx).Debugf("Getting ticketis %d notes", len(coins))
  return nil, nil
}

func (v *Convert) GetSingleTicketSky(ctx context.Context, cc *cloudcoin.CloudCoin, ridx int) (string, error) {
  ...
  pownArray, rdata := v.ProcessGenericResponsesCommon(ctx, nil, results, v.MixedConvertSuccessFunction, cce)
...
func (v *Convert) ProcessConvert(ctx context.Context, coins []cloudcoin.CloudCoin) ([]string, error) {

  isSky := false
  for _, cc:= range(coins) {
    logger.L(ctx).Debugf("Processing coinssss %d isSky (%v)",cc.Sn, cc.IsIDCoin())
    isSky = cc.IsIDCoin()
  }
  ...
  pownArray, rdata := v.ProcessGenericResponsesCommon(ctx, coins, results, v.MixedConvertSuccessFunction, cce)
  ...
  logger.L(ctx).Debug("Convert Results")
...
func (v *Convert) ReadTicket(ctx context.Context, idx int, data []byte) (int, []byte) {
  needMoreBytes := 4
...
func (v *Convert) MixedConvertSuccessFunction(ctx context.Context, idx int, status int, coins []cloudcoin.CloudCoin, rdata []byte) (int, interface{}) {
