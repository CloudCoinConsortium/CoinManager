...
func NewPown(progressChannel chan interface{}) *Pown {
	return &Pown{
		*NewServant(progressChannel),
	}
}

func (v *Pown) GetStrideSize() int {
  // packetSize = 1024 - header(22) - challenge(16) - signature(2)
  coinsLen := 1024 - 22 - 16 - 2

  // Sn + An + Pan
  coinItemSize := 3 + 16 + 16

  return coinsLen / coinItemSize
}

func (v *Pown) Pown(ctx context.Context, coins []cloudcoin.CloudCoin) (*PownOutput, error) {
  stride := v.GetStrideSize()
	logger.L(ctx).Debugf("Powning %d notes (%d notes per packet)", len(coins), stride)
  ...
  for i := 0; i < len(coins); i += stride {
    ...
    response, err := v.ProcessPown(ctx, coins[i:max])
    if err != nil {
...
func (v *Pown) ProcessPown(ctx context.Context, coins []cloudcoin.CloudCoin) (*PownOutput, error) {
  isSky := false
  for _, cc:= range(coins) {
  ...
  // Iterate over raida servers
    ...
    if isSky {
    	params[idx] = v.GetHeaderSky(COMMAND_POWN, idx, cce)
    } else {
    	params[idx] = v.GetHeader(COMMAND_POWN, idx, cce)
    }
    encb := make([]byte, 0)
    ...
    for _, cc := range(coins) {
      ...
      panData, _ := hex.DecodeString(cc.Pans[idx])

      encb = append(encb, cbyte...)
      encb = append(encb, data...)
      encb = append(encb, panData...)
    }
  ...
  pownArray, tickets := v.ProcessGenericResponsesCommon(ctx, coins, results, v.MixedPownSuccessFunction, cce)
  ...
  logger.L(ctx).Debug("Pown Results")
...
func (v *Pown) ReadTicket(ctx context.Context, idx int, data []byte) (int, []byte) {
  needMoreBytes := 4
...
func (v *Pown) MixedPownSuccessFunction(ctx context.Context, idx int, status int, coins []cloudcoin.CloudCoin, rdata []byte) (int, interface{}) {
