...
func File(title, filter string, directory bool, pt unsafe.Pointer) (string, bool, error) {
	if directory {
		out, ok := dirDialog(title, pt)
		return out, ok, nil
	}

	out, ok := fileDialog(title, filter, false, pt)
	return out, ok, nil
}
...
// extensions by spaces and use "*.extension" format for cross-platform compatibility, e.g. "*.png *.jpg".
// A blank string for the filter will display all file types.
func FileMulti(title, filter string, pt unsafe.Pointer) ([]string, bool, error) {
	out, ok := fileDialog(title, filter, true, pt)
...
func fileDialog(title, filter string, multi bool, pt unsafe.Pointer) (string, bool) {
  ...
  ofn.hwndOwner = syscall.Handle(pt)
...
func dirDialog(title string, pt unsafe.Pointer) (string, bool) {
	var bi browseinfoW
	buf := make([]uint16, maxPath)
