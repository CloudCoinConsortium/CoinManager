...
#TASKID=$(curl 'http://localhost:8888/api/v1/import' -d @<(echo "{\"name\":\"ddd\", \"items\":[{\"type\":\"inline\", \"data\":\"$b\"},   {\"type\":\"file\", \"data\": \"/home/alexander/800.CloudCoin.rn.stack\"}   ]}") 2>/dev/null| jq -r '.payload.id')
#TASKID=$(curl 'http://localhost:8888/api/v1/import' -d @<(echo "{\"name\":\"Default\", \"items\":[ {\"type\":\"file\", \"data\": \"/home/alexander/dev/superraida/coins/10000.bin\"}   ]}") 2>/dev/null| jq -r '.payload.id')
TASKID=$(curl 'http://localhost:8888/api/v1/import' -d @<(echo "{\"name\":\"Default\", \"items\":[ {\"type\":\"file\", \"data\": \"/home/alexander/ttt/2.CloudCoin.xxx.png\"}   ]}") 2>/dev/null| jq -r '.payload.id')
