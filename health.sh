...
#TASKID=$(curl 'http://localhost:8888/api/v1/health' -d '{"name":"Default"}' 2>/dev/null| jq -r '.payload.id')
TASKID=$(curl 'http://localhost:8888/api/v1/health' -d '{"name":"ddd331"}' 2>/dev/null| jq -r '.payload.id')
